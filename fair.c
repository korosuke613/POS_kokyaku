...
int fair_detailprint(ThreadParameter *threadParam, int fair_id){
	char sendBuf[BUFSIZE];
	int  sendLen;
	pthread_t selfId;                      //自分自身のスレッドID
	...
	//顧客の最大人数を取得
	//SQLコマンド実行
	sprintf(sql, "SELECT * FROM fair_detail WHERE fair_id = %d ORDER BY goods_id", fair_id);
	res = PQexec(threadParam->con, sql);
	...
	sprintf(sendBuf, "フェアID|グッズID|ポイント率%s", ENTER);
	sendLen = strlen(sendBuf); 
	send(threadParam->soc, sendBuf, sendLen, 0);
	sendBuf[sendLen-1] = '\0'; 
	printf("[C_THREAD %u] SEND=> %s\n", selfId, sendBuf); 
...
int fair_detailinsert(ThreadParameter *threadParam, int fair_id, int goods_id, double point_rate){
	char sendBuf[BUFSIZE];
	int  sendLen;
	pthread_t selfId;                      //自分自身のスレッドID
	...
	sprintf(sql, "INSERT INTO fair_detail VALUES(%d, %d, %lf)",
			fair_id,
			goods_id,
			point_rate
		   );
...
int fair_detaildelete(ThreadParameter *threadParam, int fair_id, int goods_id){
	char sendBuf[BUFSIZE];
	int  sendLen;
	pthread_t selfId;                      //自分自身のスレッドID
	...
	sprintf(sql, "DELETE FROM  fair_detail WHERE fair_id = %d AND goods_id = %d",
			fair_id,
			goods_id
		   );
	res = PQexec(threadParam->con, sql);
